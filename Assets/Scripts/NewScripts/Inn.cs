using UnityEngine;
using System.Collections.Generic;
using System.Collections;


public static class  Inn
{

    public static int playerScore_net=0;//current net profit for player
	public static int playerScore_now=0;//current day profit for player
    public static int opponentScore=0;//current profit for cpu
   
    public static int questValue=30;//monetary value for quest
    public static int bedValue=10;//monetary value for bed
    public static int foodValue=5;//monetary value for food

    public static int day=1; //current day
    public static int hour=3;//current hour
    public static int minute=00;//current minute

    public static int patronsInDay;



}